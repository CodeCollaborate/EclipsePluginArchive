...
/**
 * 
 * Modified to return positions of changes when applying patches for use when
 * live-editing (CodeCollaborate)
 * 
 * Modified methods: patch_apply Inserted class:
 * 
 * @author Modified by Benedict Wong 11/01/2015
 *
 */
public class DiffMatchPatch {
	...
	if (patches.isEmpty()) {
	    return new Object[] { text, new boolean[0], new PatchRange[0] };
	}
	...
	PatchRange[] ranges = new PatchRange[patches.size()];
	...
	for (Patch aPatch : patches) {
		...
		if (text1.equals(text2)) {
		    ...
		    String replacedText = text1;
		    String replacementText = this.diff_text2(aPatch.diffs);
		    int prefixLength = aPatch.diffs.get(0).text.length();
		    int suffixLength = aPatch.diffs.get(2).text.length();

		    // Safety net
		    if (aPatch.diffs.get(1).operation == Operation.EQUAL) {
			prefixLength = 0;
			suffixLength = aPatch.diffs.get(1).text.length();
		    }

		    switch (aPatch.diffs.get(1).operation) {
		    case INSERT:
			// Factor in nullPadding at beginning and end
			startPos = start_loc + prefixLength;
			endPos = start_loc + replacementText.length() - suffixLength;
			break;
		    case DELETE:
			// Factor in nullPadding at beginning and end
			startPos = start_loc + prefixLength;
			endPos = start_loc + replacedText.length() - suffixLength;
			break;
		    default:
			break;
		    }
			...
			for (Diff aDiff : aPatch.diffs) {
				...
				if (aDiff.operation == Operation.INSERT) {
				    // Insertion
				    startPos = start_loc + index2;
				    endPos = startPos + aDiff.text.length();
				    ...
				    // Deletion
				    startPos = start_loc + index2;
				    endPos = start_loc + this.diff_xIndex(diffs, index1 + aDiff.text.length());
		...
		startPos -= nullPadding.length();
		endPos = Math.min(text.length() - (nullPadding.length() * 2), endPos - nullPadding.length());
		if (endPos > text.length() - 5) {
		    endPos -= nullPadding.length() - (text.length() - endPos);
		}
		// add to ranges

		ranges[x] = results[x] ? new PatchRange(startPos, endPos) : null;
	    }
	    x++;
	}
	// Strip the padding off.
	text = text.substring(nullPadding.length(), text.length() - nullPadding.length());
	return new Object[] { text, results, ranges };
    }
    ...
    public static class PatchRange implements Comparable<PatchRange> {
